...
template_service = EnhancedPortfolioTemplateService()
...
[data-theme="dark"] .project-card {
    ...
    def _get_creative_designer_html(self) -> str:
        return '''<!DOCTYPE html><html><head><title>{{full_name}} - Creative</title></head>
        <body><h1>{{full_name}}</h1><p>{{summary}}</p></body></html>'''

    def _get_creative_designer_css(self) -> str:
        return '''body { font-family: 'Poppins', sans-serif; color: #2d3436; }'''
    ...
    def _get_tech_developer_html(self) -> str:
        return '''<!DOCTYPE html><html><head><title>{{full_name}} - Developer</title></head>
        <body style="background:#0d1117;color:#fff;"><h1>{{full_name}}</h1><p>{{summary}}</p></body></html>'''

    def _get_tech_developer_css(self) -> str:
        return '''body { font-family: 'Fira Code', monospace; background: #0d1117; color: #c9d1d9; }'''
    ...
    def _get_minimal_clean_html(self) -> str:
        return '''<!DOCTYPE html><html><head><title>{{full_name}}</title></head>
        <body><h1>{{full_name}}</h1><p>{{summary}}</p></body></html>'''

    def _get_minimal_clean_css(self) -> str:
        return '''body { font-family: 'Inter', sans-serif; line-height: 1.6; color: #111827; }'''
    ...
    def _get_corporate_executive_html(self) -> str:
        return '''<!DOCTYPE html><html><head><title>{{full_name}} - Executive</title></head>
        <body><h1>{{full_name}}</h1><p>{{summary}}</p></body></html>'''

    def _get_corporate_executive_css(self) -> str:
        return '''body { font-family: 'Inter', sans-serif; color: #1e293b; }'''
...
@portfolio_router.get("/enhanced-templates")
...
@portfolio_router.get("/enhanced-templates/{template_id}")
...
@portfolio_router.post("/generate-enhanced")
async def generate_enhanced_portfolio(
    ...
    try:
        ...
        portfolio_data = {
            ...
            "title": f"{resume_data.get('full_name', 'Portfolio')} - Portfolio",
